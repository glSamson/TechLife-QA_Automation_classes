...
function loadQuestion() {
    ...
    const choices = shuffleArrey(currentQuestionData.choices);
...
function checkAnswere(e){
    ...
    if(e.innerText === questions[currentQuestion].answer){
        score++;
...
choiceElements.array.forEach((btn) => {
    btn.addEventListener("click", checkAnswere)
});

nextButton.addEventListener("click", ()=> {
    if(!answerChosen) {
        alert("Por favor, responda uma pergunta!")
        return;
    }
    currentQuestion++;
    ...
    if(currentQuestion<questions.length){
        ...
        alert(
            `Fim de jogo! Você acertou ${score} de ${question.length} perguntas.`
        )
    restartQuiz()
    }
    }
...
loadQuestion();         // todo quebrado, última linha responsiva: 70